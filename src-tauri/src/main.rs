...
use aim_lib::paths;
use aim_lib::server::instances::{AppState, InstancesCache, PendingFocus, PsCache};
use aim_lib::server::summarizer::Summarizer;
...
async fn main() {
    ...
    if let Some(ref res) = resource_dir {
        ...
        if hook_src.exists() {
            aim_lib::hooks::install(&hook_src);
        }
    ...
    let router = aim_lib::server::build_router(state.clone());
